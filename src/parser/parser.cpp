...
ast::ASTNode* Parser::parse_create_stmt() {
    ...
    if (create_type == "TABLE" || create_type == "table" ||
        ...
        // Now we're at TABLE
        return parse_create_table_full();  // Use full implementation
    } else if (create_type == "INDEX" || create_type == "index" ||
               create_type == "UNIQUE" || create_type == "unique") {
        // Handle CREATE [UNIQUE] INDEX
        return parse_create_index_full();  // Use full implementation
        ...
        // Reset position to CREATE
        current_token_ = start_token;
        tokenizer_->set_position(start_pos);
    ...
    if (current_token_ && current_token_->type == tokenizer::TokenType::Keyword) {
        ...
        if (action == "ADD" || action == "add") {
            ...
            if (current_token_ && current_token_->keyword_id == db25::Keyword::TO) {
                advance();
            } else if (current_token_ && current_token_->keyword_id == db25::Keyword::COLUMN) {
                advance();
            }
...
ast::ASTNode* Parser::parse_primary_expression() {
    ...
    if (current_token_->type == tokenizer::TokenType::Identifier ||
        ...
        // In SELECT, WHERE, GROUP BY, HAVING, ORDER BY contexts, identifiers are column references
