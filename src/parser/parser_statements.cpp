...
ast::ASTNode* Parser::parse_returning_clause() {
    ...
    auto* returning_node = arena_.allocate<ast::ASTNode>();
    new (returning_node) ast::ASTNode(ast::NodeType::ReturningClause);
    returning_node->node_id = next_node_id_++;
    ...
    if (current_token_ && current_token_->value == "*") {
        ...
        star->parent = returning_node;
        returning_node->first_child = star;
        returning_node->child_count = 1;
        advance();
    } else {
        ...
        do {
            ...
            if (current_token_ && (current_token_->keyword_id == db25::Keyword::AS ||
                ...
                if (current_token_->keyword_id == db25::Keyword::AS) {
                    advance(); // consume AS
                }
                if (current_token_ && current_token_->type == tokenizer::TokenType::Identifier) {
                    expr->catalog_name = copy_to_arena(current_token_->value); // Store alias
                    expr->flags = static_cast<ast::NodeFlags>(
                        static_cast<uint8_t>(expr->flags) | static_cast<uint8_t>(ast::NodeFlags::HasAlias)
                    );
                    advance();
                }
            }
            
            expr->parent = returning_node;
            if (!first_expr) {
                first_expr = expr;
                returning_node->first_child = first_expr;
            } else if (last_expr) {
                last_expr->next_sibling = expr;
            }
            last_expr = expr;
            returning_node->child_count++;
...
ast::ASTNode* Parser::parse_on_conflict_clause() {
    ...
    auto* conflict_node = arena_.allocate<ast::ASTNode>();
    new (conflict_node) ast::ASTNode(ast::NodeType::OnConflictClause);
    conflict_node->node_id = next_node_id_++;
    
    // Parse optional column list: ON CONFLICT (col1, col2)
    if (current_token_ && current_token_->value == "(") {
        advance(); // consume (
        parenthesis_depth_++;
        ...
        column_list->parent = conflict_node;
        conflict_node->first_child = column_list;
        conflict_node->child_count = 1;
    ...
    if (current_token_ && current_token_->keyword_id == db25::Keyword::DO) {
        ...
        if (current_token_ && current_token_->value == "NOTHING") {
            // DO NOTHING
            conflict_node->flags = static_cast<ast::NodeFlags>(
                static_cast<uint8_t>(conflict_node->flags) | 0x01  // Custom flag for DO NOTHING
            );
            ...
            if (current_token_ && current_token_->keyword_id == db25::Keyword::SET) {
                ...
                auto* set_list = arena_.allocate<ast::ASTNode>();
                new (set_list) ast::ASTNode(ast::NodeType::SetStmt);
                set_list->node_id = next_node_id_++;
                set_list->parent = conflict_node;
                
                // Link to conflict node
                if (conflict_node->first_child) {
                    conflict_node->first_child->next_sibling = set_list;
                } else {
                    conflict_node->first_child = set_list;
                }
                conflict_node->child_count++;
                
                // Parse column = expression pairs
                ast::ASTNode* first_assign = nullptr;
                ast::ASTNode* last_assign = nullptr;
                ...
                do {
                    ...
                    if (current_token_ && current_token_->value == "=") {
                        ...
                        if (expr) {
                            ...
                            auto* assign = arena_.allocate<ast::ASTNode>();
                            new (assign) ast::ASTNode(ast::NodeType::BinaryExpr);
                            assign->node_id = next_node_id_++;
                            assign->primary_text = copy_to_arena("=");
                            assign->parent = set_list;
                            
                            col->parent = assign;
                            expr->parent = assign;
                            assign->first_child = col;
                            col->next_sibling = expr;
                            assign->child_count = 2;
                            
                            if (!first_assign) {
                                first_assign = assign;
                                set_list->first_child = first_assign;
                            } else if (last_assign) {
                                last_assign->next_sibling = assign;
                            }
                            last_assign = assign;
                            set_list->child_count++;
                        }
                    }
                ...
                if (current_token_ && current_token_->keyword_id == db25::Keyword::WHERE) {
                    ...
                    if (where) {
                        where->parent = conflict_node;
                        set_list->next_sibling = where;
                        conflict_node->child_count++;
                    }
                }
            }
...
ast::ASTNode* Parser::parse_using_clause() {
    // Parse USING clause for JOINs or DELETE statements
    auto* using_node = arena_.allocate<ast::ASTNode>();
    new (using_node) ast::ASTNode(ast::NodeType::UsingClause);
    using_node->node_id = next_node_id_++;
    ...
    if (current_token_ && current_token_->value == "(") {
        ...
        while (current_token_ && current_token_->type == tokenizer::TokenType::Identifier) {
            ...
            col->parent = using_node;
            
            if (!first_col) {
                first_col = col;
                using_node->first_child = first_col;
            } else if (last_col) {
                last_col->next_sibling = col;
            }
            last_col = col;
            using_node->child_count++;
